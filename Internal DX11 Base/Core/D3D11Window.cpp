...
namespace DX11Base {
    namespace Core {
        ...
        LRESULT D3D11Window::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) 
        {
            ...
            {
                ImGui_ImplWin32_WndProcHandler((HWND)Core::g_D3D11Window->m_OldWndProc, msg, wParam, lParam);
                return true;
            }
            return CallWindowProc((WNDPROC)Core::g_D3D11Window->m_OldWndProc, hWnd, msg, wParam, lParam);
        }

        HRESULT APIENTRY D3D11Window::SwapChain_Present_hook(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
        {
            Core::g_D3D11Window->Overlay(pSwapChain);

            return Core::g_D3D11Window->IDXGISwapChain_Present_stub(pSwapChain, SyncInterval, Flags);
        }
        ...
        HRESULT APIENTRY D3D11Window::SwapChain_ResizeBuffers_hook(IDXGISwapChain* p, UINT bufferCount, UINT Width, UINT Height, DXGI_FORMAT fmt, UINT scFlags)
        {
            ...
            Core::g_D3D11Window->m_pSwapChain = p;
            Core::g_D3D11Window->m_RenderTargetView->Release();
            Core::g_D3D11Window->m_RenderTargetView = nullptr;

            //  get fn result
            HRESULT result = Core::g_D3D11Window->IDXGISwapChain_ResizeBuffers_stub(p, bufferCount, Width, Height, fmt, scFlags);
            ...
            if (backBuffer)
            {
                Core::g_D3D11Window->m_Device->CreateRenderTargetView(backBuffer, 0, &Core::g_D3D11Window->m_RenderTargetView);
                backBuffer->Release();
            }

            //  Reset ImGui 
            if (Core::g_D3D11Window->bInitImGui)
            {
                ImGuiIO& io = ImGui::GetIO();
